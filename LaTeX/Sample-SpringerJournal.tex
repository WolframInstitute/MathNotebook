% LaTeX counterpart of the SpringerJournal stylesheet sample.
% svjour3.cls is distributed by Springer, not by TeX Live:
...
\journalname{A Springer Journal}

\begin{document}

\title{A Sample Article}
\author{MathNotebook}
\institute{Set with the \texttt{svjour3} class, in Times.}
\date{\today}

\maketitle

\begin{abstract}
The same document as the SpringerJournal notebook sample, typeset in LaTeX, so the
...
\begin{proof}
Summing degrees counts every edge twice. \qed
\end{proof}
