...
- **Next:** T6 stays open, and Pavel chose to finish it next: build the column width as an **absolute, centered** point measure with a `Full` position (his decision, recorded below), and fix the palette rendering dark regardless of theme (point 4 — needs one diagnostic from him first: `CurrentValue[ $FrontEnd, LightDark ]` against the notebook's own value).
