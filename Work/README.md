...
- [LaTeX Paper Import](Active/LaTeXPaperImport.md) — T1–T9 done (`ImportLaTeXDocument`/`ExportLaTeXDocument`: both specimen papers round-trip byte-identically — through a real save and reopen — with sectioning, theorem environments, labels, cross-references, citations, the `.bib` bibliography, figures, front matter, lists and **numbering** converted; the number the notebook shows is now the number LaTeX prints, read out of `\newtheorem`, `\numberwithin`, the document class and enumitem's `label=`); next: **T10**, a `thebibliography` written into the `.tex`, and a report when a declared `.bib` is missing
