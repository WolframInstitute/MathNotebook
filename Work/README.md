...
- [LaTeX Paper Import](Active/LaTeXPaperImport.md) — T1–T5 done (`ImportLaTeXDocument`/`ExportLaTeXDocument`: both specimen papers round-trip byte-identically — now through a real save and reopen — with sectioning, theorem environments, labels, cross-references, citations, the `.bib` bibliography and figures converted; the causal paper renders as a 10-page PDF with a live `WolframModel` causal graph in it); next: **T6**, make both papers round-trip test fixtures
