...
- [LaTeX Paper Import](Active/LaTeXPaperImport.md) — T1–T6 done (`ImportLaTeXDocument`/`ExportLaTeXDocument`: both specimen papers round-trip byte-identically — through a real save and reopen — with sectioning, theorem environments, labels, cross-references, citations, the `.bib` bibliography and figures converted; the causal paper renders as a 10-page PDF with a live `WolframModel` causal graph in it; both papers are now fixtures in `Tests/Specimens.wlt`, which finds them rather than shipping them); next: **T7**, display math and nested environments inside a theorem body
