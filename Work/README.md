...
- [Inline Math Converter Defects](Active/InlineMathConverterDefects.md) — T1–T2 done (the comma defect was really "the fragment must be one Wolfram expression"; the starred-environment defect was really "display math inside a paragraph is never split out" — the specimen paper's 35 unconverted spans and both missed display equations are now 0); next: **T3**, protected single letters (`E`, `I`)
