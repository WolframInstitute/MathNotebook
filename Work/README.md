...
**0.1.15 is the published version** (2026-07-28) — 265 tests green, `Scripts/PublishPaclet.wls` then
`Scripts/DeployPreviews.wls`, marker and archive verified from the cloud (281 KB, `"Version" -> "0.1.15"`),
installed and driven headless: the T2 and T3 guards answer their dialogs from the installed paclet and
all 22 reference pages agree with `Kernel/Usage.wl`. It carries the `BasicFunctionality` guards on
twelve entry points' worth of `$Failed` states, and the reference pages regenerated by
`Scripts/RegenerateUsage.wls`. **The documentation is deployed nowhere but inside the paclet** — the
browser mirror was deleted at Pavel's call on 2026-07-28. 0.1.13 (2026-07-28) was the submission
bundle, the `ComplexSystems` template that 0.1.12 generated but did not ship, and a 22nd reference page.
