...
- [LaTeX Paper Import](Active/LaTeXPaperImport.md) — T1–T7 done (`ImportLaTeXDocument`/`ExportLaTeXDocument`: both specimen papers round-trip byte-identically — through a real save and reopen — with sectioning, theorem environments, labels, cross-references, citations, the `.bib` bibliography and figures converted; an environment body is now split like the document body, so hodgepaper's 55 display blocks are 2 and its 72 literal `\ref`s are 29; both papers are fixtures in `Tests/Specimens.wlt`, which finds them rather than shipping them); next: **T8**, front matter and lists
