...
- [LaTeX Paper Import](Active/LaTeXPaperImport.md) — T1–T10 done (`ImportLaTeXDocument`/`ExportLaTeXDocument`: both specimen papers **and the repo's four LaTeX samples** round-trip byte-identically — through a real save and reopen — with sectioning, theorem environments, labels, cross-references, citations, numbering, figures, front matter, lists and **both bibliographies** converted; a `thebibliography` in the `.tex` gives entries the notebook owns and writes back, a `.bib` gives entries it shows and leaves alone, and a declared `.bib` that is not there is now reported); next: **T11**, an imported paper opens with its environments live — a fifth stylesheet, and a sheet chosen from the `\documentclass`
