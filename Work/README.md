...
- [LaTeX Paper Import](Active/LaTeXPaperImport.md) — T1–T8 done (`ImportLaTeXDocument`/`ExportLaTeXDocument`: both specimen papers round-trip byte-identically — through a real save and reopen — with sectioning, theorem environments, labels, cross-references, citations, the `.bib` bibliography, figures, front matter and lists converted; every one of the two papers' 72 items is a cell that shows its marker, and the only LaTeX left in either paper's prose is 29 references with no target, 2 unparsable display blocks, 6 commented-out `\item`s and `\maketitle`); next: **T9**, numbering — the causal paper numbers per subsection where the stylesheets number per section
